...
const double T0 = 300.0;    // In K, will be scaled using kb and eps
...
void printLattice(bool reduced, double pos[N][N][N][4][3])
{
  ...
  if (!reduced) {
    std::cout << "x, y, z coordinates printing to lattice.txt, in meters \n";
    outfile << "x, y, z coordinates, in meters \n";
  } else {
    std::cout << "x, y, z coordinates printing to lattice.txt, reduced units \n";
...
{
  std::ofstream outforce("forces.txt");
  ...
  if (!reduced) {
    std::cout << "x, y, z force components printing to forces.txt, in Newtons \n";
    outforce << "x, y, z force components, in Newtons \n";
  } else {
    std::cout << "x, y, z force components printing to forces.txt, reduced units \n";
...
int main(int argc, char const *argv[])
{
  ...
  double dt[3] = {0.001,0.005,0.02};
  int step[3] = {10001,2001,501};
  double out1[4][step[0]]; // At each time step:
  double out2[4][step[1]]; // time, Ek, Ep, Et (per atom)
  double out3[4][step[2]];
  out1[0][0] = 0.0; out2[0][0] = 0.0; out3[0][0] = 0.0; //time
  out1[1][0] = KE;  out2[1][0] = KE;  out3[1][0] = KE;  //kinetic energy
  out1[2][0] = PE;  out2[2][0] = PE;  out3[2][0] = PE;  //periodic LJ potential
  out1[3][0]=KE+PE; out2[3][0]=KE+PE; out3[3][0]=KE+PE; //Total energy
  std::cout << "\nKE/atom (eV) start: \t" << KE << " \n";
  std::cout << "PE/atom (eV) start: \t" << PE << " \n";

  std::ofstream out05001("out05001.txt");
  std::ofstream out05005("out05005.txt");
  std::ofstream out05020("out05020.txt");
  out05001 << std::scientific << std::setprecision(6);
  out05005 << std::scientific << std::setprecision(6);
  out05020 << std::scientific << std::setprecision(6);
  out05001 << out1[0][0] << "\t" << out1[1][0] << "\t"<< out1[2][0] << "\t"<< out1[3][0] << std::endl;
  out05005 << out2[0][0] << "\t" << out2[1][0] << "\t"<< out2[2][0] << "\t"<< out2[3][0] << std::endl;
  out05020 << out3[0][0] << "\t" << out3[1][0] << "\t"<< out3[2][0] << "\t"<< out3[3][0] << std::endl;

  //Verlet iteration
  std::cout << "\nWorking on dt=0.001 for 10LJ time \n";
  for (int k=1; k<step[0]; k++) {
    ...
    if (k % 1000 == 0) {
      out05001 << out1[0][k] << "\t" << out1[1][k] << "\t"<< out1[2][k] << "\t"<< out1[3][k] << std::endl;
      std::cout << k/100 << "\% \t"; std::cout.flush(); //Flush, progress every 10%
    } else {
      out05001 << out1[0][k] << "\t" << out1[1][k] << "\t"<< out1[2][k] << "\t"<< out1[3][k] << " \n";
    }
  }

  //Reset and compute for new time step
  createLattice(pos,force);
  std::cout << "\n";
  initVelocity(v);
  calcSystemLJ(true,p,f);
  std::cout << "Working on dt=0.005 for 10LJ time \n";
  for (int k=1; k<step[1]; k++) {
    velocityStep(v,f,dt[1]);              //First velocity halfstep
    positionStep(p,v,dt[1]);              //Set position for next iteration
    resetForce(force);
    out2[2][k] = calcSystemLJ(true,p,f);  //Also performs initForce (update acc)
    out2[1][k] = velocityStep(v,f,dt[1]); //Set velocity for next iteration
    out2[0][k] = out2[0][k-1] + tau*dt[1];//Update timee
    out2[3][k] = out2[1][k] + out2[2][k]; //Update total E per atom
    if (k % 200 == 0) {
      out05005 << out2[0][k] << "\t" << out2[1][k] << "\t"<< out2[2][k] << "\t"<< out2[3][k] << std::endl;
      std::cout << (int)k/20 << "\% \t"; std::cout.flush(); //Flush, progress every 10%
    } else {
      out05005 << out2[0][k] << "\t" << out2[1][k] << "\t"<< out2[2][k] << "\t"<< out2[3][k] << " \n";
    }
  }
  ...
  initVelocity(v);
  calcSystemLJ(true,p,f);
  std::cout << "Working on dt=0.020 for 10LJ time \n";
  for (int k=1; k<step[2]; k++) {
    velocityStep(v,f,dt[2]);              //First velocity halfstep
    positionStep(p,v,dt[2]);              //Set position for next iteration
    resetForce(force);
    out3[2][k] = calcSystemLJ(true,p,f);  //Also performs initForce (update acc)
    out3[1][k] = velocityStep(v,f,dt[2]); //Set velocity for next iteration
    out3[0][k] = out3[0][k-1] + tau*dt[2];//Update time
    out3[3][k] = out3[1][k] + out3[2][k]; //Update total E per atom
    if (k%50 == 0) {
      out05020 << out3[0][k] << "\t" << out3[1][k] << "\t"<< out3[2][k] << "\t"<< out3[3][k] << std::endl;
      std::cout << (int)k/5 << "\% \t"; std::cout.flush(); //Flush, progress every 10%
    } else {
      out05020 << out3[0][k] << "\t" << out3[1][k] << "\t"<< out3[2][k] << "\t"<< out3[3][k] << " \n";
    }
  }
  std::cout << "\n\nOutput energies printed to out0Nttt.txt \n";
